...
export const openStats = async () => {
  ...
  const allMutations = [
    ...MUTATION_PRIORITY,
    ...Object.keys(mutationCounts).filter((m) => !MUTATION_PRIORITY.includes(m))
  ]
...
const onWorldSelect = () => {
  ...
  if (config.num_food_clusters !== undefined) {
    document.getElementById('cfg-num-food-clusters').value =
      config.num_food_clusters
    document.getElementById('cfg-num-food-clusters-range').value =
      config.num_food_clusters
  }

  if (config.food_peak_probability !== undefined) {
    document.getElementById('cfg-food-peak-probability').value =
      config.food_peak_probability
    document.getElementById('cfg-food-peak-probability-range').value =
      config.food_peak_probability
  }
}
