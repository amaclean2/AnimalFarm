...
let CELL = mobileQuery.matches ? CELL_MOBILE : CELL_DESKTOP

const panel = document.getElementById('panel')

function resize() {
  CELL = mobileQuery.matches ? CELL_MOBILE : CELL_DESKTOP
...
function clampCamera() {
  ...
  camY = Math.max(
    0,
    Math.min(Math.max(0, WORLD_H * CELL - canvas.height), camY)
  )
}
...
canvas.addEventListener(
  ...
  (e) => {
    if (e.touches.length !== 1) return
    camX -= e.touches[0].clientX - touchLastX
    camY -= e.touches[0].clientY - touchLastY
    clampCamera()
    touchLastX = e.touches[0].clientX
    touchLastY = e.touches[0].clientY
    touchMoved = true
...
canvas.addEventListener('touchend', (e) => {
  ...
  const wx = Math.floor((touch.clientX - rect.left + camX) / CELL)
  const wy = Math.floor((touch.clientY - rect.top + camY) / CELL)
  let hit = null
...
async function fetchState() {
  ...
  worldData.agents.forEach((a) =>
    a.alive ? upsertAgent(a) : deadAgents.set(a.id, a)
  )
...
function abbrevId(val) {
  ...
  if (Array.isArray(val))
    return val
      .map((v) => (UUID_RE.test(String(v)) ? String(v).slice(0, 8) : v))
      .join(', ')
...
async function openStats() {
  ...
  for (const [key, rows] of arrays) {
    ...
    if (!rows.length) {
      ...
      const maxAge = cols.includes('age')
        ? Math.max(...rows.map((r) => r.age ?? 0))
        : 0
...
function drawStockpiles() {
  for (const g of groups.values()) {
    ...
    const cx = sx + CELL / 2
    const cy = sy + CELL / 2
    const half = CELL * 0.23

    ctx.fillStyle =
      g.stockpile > 0 ? 'rgba(232, 180, 80, 0.85)' : 'rgba(80, 80, 80, 0.6)'
    ...
    ctx.font = `bold ${Math.max(7, Math.round(CELL * 0.3))}px Roboto, sans-serif`
...
function drawAgents() {
  for (const a of deadAgents.values()) {
    ...
    ctx.beginPath()
    ctx.arc(sx + CELL / 2, sy + CELL * 0.37, CELL * 0.3, 0, Math.PI * 2)
    ctx.fillStyle = '#444'
    ctx.fill()
  ...
  for (const a of agents.values()) {
    ...
    if (a.carrying_food) {
      ctx.beginPath()
      ctx.arc(cx + CELL * 0.2, cy - CELL * 0.2, CELL * 0.1, 0, Math.PI * 2)
      ctx.fillStyle = '#f1c40f'
      ctx.fill()
    }

    const barW = CELL * 0.8
    const barX = sx + CELL * 0.1
    const barY = sy + CELL * 0.73
    ctx.fillStyle = '#222'
...
function frame(now) {
  ...
  const visionR = visionAgent
    ? visionAgent.group_id
      ? Math.round(visionAgent.vision_range * 1.5)
      : visionAgent.vision_range
    : 0
  const visionFoods = new Set()
  if (visionAgent) {
    for (const f of food.values()) {
      if (
        Math.abs(f.x - visionAgent.x) + Math.abs(f.y - visionAgent.y) <=
        visionR
      ) {
