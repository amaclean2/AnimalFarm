...
class Agents:
    ...
    def find_mate_target(self, agent: Agent, tick_count: int) -> Pos | None:

        if not agent.is_eligible_to_mate(tick_count):
            return None

        best: Pos | None = None
        best_dist = float("inf")

        for other in self.in_range(agent, agent.vision_range):

            if not other.is_eligible_to_mate(tick_count):
                continue
